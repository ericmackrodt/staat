...
//         });
//       };
//       const unsubscribe = subscribe(checkForUpdates as any);
...
function areEqualShallow<TThing extends {}>(left: TThing, right: TThing) {
  ...
  return function<TSubset>(selector: (state: TState) => TSubset): TSubset {
    ...
    useEffect(() => {
      function stateChanged() {
        ...
        if (!areEqualShallow(currentState, newState)) {
          setState(newState);
        }
      ...
      return () => staat.unsubscribe(stateChanged);
    }, []);
